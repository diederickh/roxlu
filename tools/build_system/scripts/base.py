...
def rb_get_download_dir(script = None):
    if script is None:
        return os.path.abspath(config.download_dir) +"/"
    else:
...
def rb_get_script_dir(script = None):
    if script is None:
        ...
        return os.path.abspath(config.script_dir +"/" +script.name ) +"/"
...
def rb_get_download_path(script = None):
    if script is None:
        return os.path.abspath(config.base_dir +"/" +config.download_dir) +"/"
    else:
        return os.path.abspath(config.base_dir +"/" +config.download_dir +"/" +script.name ) +"/"
...
def rb_extract_file(dir, filename, ext):
    if rb_is_unix():
        if ext == "tar.gz":
            ...
            if rb_is_linux():
                os.system("cd " +dir + " && unzip " +filename)
            else:
...
def rb_compiler_string(comp): 
    if comp == config.COMPILER_MAC_GCC:
        return "mac-gcc"
    elif comp == config.COMPILER_MAC_CLANG:
        return "mac-clang"
    elif comp == config.COMPILER_WIN_MSVC2010:
        return "win-vs2010"
    elif comp == config.COMPILER_WIN_MSVC2012:
        return "win-vs2012"
    elif comp == config.COMPILER_UNIX_GCC:
        return "unix-gcc"
    elif comp == config.COMPILER_UNIX_CLANG:
        return "unix-clang"

def rb_get_compiler_shortname():
    if config.compiler == config.COMPILER_MAC_GCC:
       return "gcc"
    elif config.compiler == config.COMPILER_WIN_MSVC2010:
        return "vs2010"
    elif config.compiler == config.COMPILER_WIN_MSVC2012:
        return "vs2012"
    elif config.compiler == config.COMPILER_MAC_CLANG:
        return "clang"
    elif config.compiler == config.COMPILER_UNIX_GCC:
        return "gcc"
    elif config.compiler == config.COMPILER_UNIX_CLANG:
        return "clang"
    return "none"

def rb_is_msvc():
    return config.compiler == config.COMPILER_WIN_MSVC2010 or config.compiler == config.COMPILER_WIN_MSVC2012

def rb_is_vs2010():
    return config.compiler == config.COMPILER_WIN_MSVC2010

def rb_is_vs2012():
    return config.compiler == config.COMPILER_WIN_MSVC2012

def rb_is_mac_gcc():
    return config.compiler == config.COMPILER_MAC_GCC    

def rb_is_unix_gcc():
    return config.compiler == config.COMPILER_UNIX_GCC

def rb_is_mac_clang():
    return config.compiler == config.COMPILER_MAC_CLANG

def rb_is_unix_clang():
    return config.compiler == config.COMPILER_UNIX_CLANG

def rb_is_clang():
    return rb_is_mac_clang() or rb_is_unix_clang()

def rb_is_gcc():
    return rb_is_mac_gcc() or rb_is_unix_gcc()

def rb_is_debug():
    return config.build_type == config.BUILD_TYPE_DEBUG

def rb_is_release():
    return config.build_type == config.BUILD_TYPE_RELEASE
...
def rb_is_32bit():
    return config.arch == config.ARCH_M32

def rb_is_64bit():
    return config.arch == config.ARCH_M64
...
def rb_install_get_dir():
    ...
    return os.path.normpath(config.install_prefix +"/" +rb_get_triplet() +flag) +"/"
...
def rb_arch_string(arch):
    if arch == config.ARCH_M32:
        return "32"
    elif arch == config.ARCH_M64:
        return "64"
    else:
        return "-"
...
def rb_execute_shell_commands(script, commands, environmentVars = None):
    ...
    if rb_get_os_shortname() == "win":
        ...
        f.write("\n".join(cmd))
        f.close()
        os.chmod(d + "tmp.sh", 0o7777)
        #os.chmod(d + "tmp.sh", stat.S_IREAD | stat.S_IWRITE | stat.S_IEXEC)
        os.system(d + "tmp.sh")
...
def rb_get_tools_path():
    return config.base_dir +config.tools_dir +"/"
...
def rb_msvc_get_msbuild_type_flag():
    if config.compiler == config.COMPILER_WIN_MSVC2010:
...
def rb_vs2012_get_msbuild_type_flag():
    ...
    elif config.build_type == config.BUILD_TYPE_RELEASE:
...
def rb_msvc_get_setvars():
    if config.compiler == config.COMPILER_WIN_MSVC2010:
        return config.base_dir +"/tools/vs2010_vcvars32.bat"
    elif config.compiler == config.COMPILER_WIN_MSVC2012:
        return config.base_dir +"/tools/vs2012_vcvars32.bat"
    else:
...
def rb_cmake_get_generator_name():
    if config.compiler == config.COMPILER_WIN_MSVC2010 and rb_vs2010_is_installed():
        if config.arch == config.ARCH_M64:
            return "Visual Studio 10 Win64"
        else:
            return "Visual Studio 10"
    elif config.compiler == config.COMPILER_WIN_MSVC2012 and rb_vs2012_is_installed():
        if config.arch == config.ARCH_M64:
