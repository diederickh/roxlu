...
- (id) init {
  ...
  printf("ADD @autoreleasepool where necessary!\n");        
  return self;
}
      ...
      andFrameRate:(float) fps
{
  ...
  @autoreleasepool { // tmp
    ...
    if(!cap_device) {
      printf(ERR_CM_DEVICE_NOT_FOUND);
      return 0;
    }
    ...
    if(!did_found_format) {
      printf(ERR_CM_FORMAT_NOT_FOUND);
      return 0;
    }
    ...
    if([self isPixelFormatSupportedByCaptureVideoDataOutput: output pixelFormat: fmt] == 0) {
      printf(ERR_CM_PIX_FMT);
      return 0;
    }
    [output retain];
...
#if 1
    CMPixelFormatType pixel_format_type = [self videoCapturePixelFormatToAVFoundationPixelFormat: fmt];
...
- (int) closeDevice {
  if(!session) {
    printf("Cannot close the device because it hasn't been opened yet.\n");
    return 0;
  }
...
- (void) dealloc {
  printf("DEALLOC THE VIDEOCAPTUREAVFOUNDATION\n");
  return; // tmp
...
- (int) videoCapturePixelFormatToAVFoundationPixelFormat: (VideoCapturePixelFormat) fmt {
  switch(fmt) {
    ...
    case VIDEOCAPTURE_FMT_RGB24:          return kCMPixelFormat_24RGB;                  break;
    case VIDEOCAPTURE_FMT_RGB555BE:       return kCMPixelFormat_16BE555;                break;
    case VIDEOCAPTURE_FMT_RGB565BE:       return kCMPixelFormat_16BE565;                break;
    case VIDEOCAPTURE_FMT_RGB555LE:       return kCMPixelFormat_16LE555;                break;
    case VIDEOCAPTURE_FMT_RGB565LE:       return kCMPixelFormat_16LE565;                break;
    case VIDEOCAPTURE_FMT_UYVY422:        return kCMPixelFormat_422YpCbCr8;             break;
    case VIDEOCAPTURE_FMT_YUYV422:        return kCMPixelFormat_422YpCbCr8_yuvs;        break;
    case VIDEOCAPTURE_FMT_YUV444P:        return kCMPixelFormat_444YpCbCr8;             break;
    case VIDEOCAPTURE_FMT_YUV444P16LE:    return kCMPixelFormat_4444YpCbCrA8;           break; // @todo - not sure about this one
...
- (int) avFoundationPixelFormatToVideoCapturePixelFormat: (int) fmt {
  ...
  switch(fmt) {
    ...
    case  kCMPixelFormat_32BGRA:           return VIDEOCAPTURE_FMT_BGRA;         break;
    case  kCMPixelFormat_24RGB:            return VIDEOCAPTURE_FMT_RGB24;        break;
    case  kCMPixelFormat_16BE555:          return VIDEOCAPTURE_FMT_RGB555BE;     break;
    case  kCMPixelFormat_16BE565:          return VIDEOCAPTURE_FMT_RGB565BE;     break;
    case  kCMPixelFormat_16LE555:          return VIDEOCAPTURE_FMT_RGB555LE;     break;
    case  kCMPixelFormat_16LE565:          return VIDEOCAPTURE_FMT_RGB565LE;     break;
    case  kCMPixelFormat_422YpCbCr8:       return VIDEOCAPTURE_FMT_UYVY422;      break;
    case  kCMPixelFormat_422YpCbCr8_yuvs:  return VIDEOCAPTURE_FMT_YUYV422;      break;
    case  kCMPixelFormat_444YpCbCr8:       return VIDEOCAPTURE_FMT_YUV444P;      break;
    case  kCMPixelFormat_4444YpCbCrA8:     return VIDEOCAPTURE_FMT_YUV444P16LE;  break;
    case  kCMPixelFormat_422YpCbCr16:      return VIDEOCAPTURE_FMT_YUV422P16LE;  break;
...
- (void) printSupportedPixelFormatsByVideoDataOutput: (AVCaptureVideoDataOutput*) o {
  printf("\n");
  printf("Supported pixel formats by the AVCaptureVideoDataOutput* (not same as AVCaptureDeviceInput!).\n");
  printf("--------------------------------------------------------------------\n");
...
- (void) printSupportedVideoCodecFormatsByVideoDataOutput: (AVCaptureVideoDataOutput*) o {
  printf("\n");
  printf("Supported video codec formats by the AVCaptureVideoDataOutput* (not same as AVCaptureDeviceInput!)\n");
  printf("--------------------------------------------------------------------\n");
  NSArray* supported_codecs = o.availableVideoCodecTypes;
  for(NSString *curr_codec in supported_codecs) {
    printf("\t%s\n", [curr_codec UTF8String]);
  }
}
...
std::vector<VideoCaptureCapability> avf_get_capabilities(void* cap, int device) {
  std::vector<VideoCaptureCapability> result;
